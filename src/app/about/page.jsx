          ...
          >
            <div>
              ...
              <h1 className="text-4xl sm:text-5xl font-light uppercase tracking-widest text-[#1c1a17] leading-tight font-serif">
                G. <span className="font-semibold text-neutral-500 font-serif">Venketram</span>
              </h1>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-8 border-y border-[#e6e2d8]">
              {stats.map((stat) => (
                <div key={stat.label} className="space-y-1">
                  <span className="text-2xl sm:text-3xl font-extralight text-[#1c1a17] block font-serif">
                    {stat.value}
                  </span>
            ...
            <div className="space-y-4">
              <h3 className="text-xs uppercase tracking-[0.25em] text-neutral-500 font-semibold">
                Select Client & Publications
              </h3>
              <div className="flex flex-wrap gap-x-8 gap-y-3 text-xs uppercase tracking-widest text-neutral-600 font-semibold">
                {publications.map((pub) => (
                  <span key={pub} className="hover:text-black transition-colors duration-300">
                    {pub}
                  </span>
                ))}
              </div>
          ...
          >
            <div className="relative aspect-[3/4] w-full rounded-2xl overflow-hidden border border-[#e6e2d8] shadow-lg group">
              <Image
                ...
                className="object-cover object-center grayscale hover:grayscale-0 transition-all duration-1000 ease-out scale-100 group-hover:scale-[1.03]"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#f5f2eb]/20 via-transparent to-transparent pointer-events-none" />
            </div>
            
            {/* Visual Decorative Box */}
            <div className="absolute -bottom-4 -left-4 -z-10 w-24 h-24 border-l border-b border-[#e6e2d8] rounded-bl-2xl hidden sm:block" />
            <div className="absolute -top-4 -right-4 -z-10 w-24 h-24 border-r border-t border-[#e6e2d8] rounded-tr-2xl hidden sm:block" />
          </motion.div>
